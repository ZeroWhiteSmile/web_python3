...
export default {
  ...
  watch: {
    baidumap: function (newVal, oldVal) {
      console.log('newVal', newVal)
    }
  },
  mounted() {
    // 注意：v=3.0 不是v=2.0
    let url = "http://api.map.baidu.com/api?v=3.0&ak=c1UeaC9cqD1MQNttQb7y5jNbH2GdV7Nj&callback="
    promiseUrl(url).then(value => {
      console.log('加载完成', value)
      this.initEcharts()
    }).catch(reason => {
      console.dir('加载完成', reason)
      // this.$nextTick(() => {
      //   this.initEcharts()
      // })
    })
  ...
  methods: {
    initEcharts() {
      ...
      // 百度地图API功能
      // 获取百度地图实例，使用百度地图自带的控件
      this.baidumap = myCharts.getModel().getComponent('bmap').getBMap()
      // 控件 - 地图类型 => 地图、三维、微信
      this.baidumap.addControl(new BMap.MapTypeControl())
      // 定位 - 浏览器定位 => 中心点和缩放级别
      this.baidumap.centerAndZoom(new BMap.Point(105.755971, 35.713033), 6)
