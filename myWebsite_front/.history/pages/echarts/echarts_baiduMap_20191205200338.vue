...
export default {
  ...
  mounted() {
    // 注意：v=3.0 不是v=2.0
    let url = "http://api.map.baidu.com/api?v=3.0&ak=c1UeaC9cqD1MQNttQb7y5jNbH2GdV7Nj&callback=init"
    promiseUrl(url, 'BMap').then(value => {
      console.log(value)
      this.initEcharts()
    }).catch(reason => {
  ...
  methods: {
    initEcharts() {
      ...
      let baidumap = myCharts.getModel().getComponent('bmap').getBMap()
      // 控件 - 地图类型 => 地图、三维、微信
      baidumap.addControl({ new BMap.MapTypeControl(), new BMap.disableDoubleClickZoom() })
      // 定位 - 浏览器定位 => 中心点和缩放级别
      ...
      // 禁用双击放大
      // baidumap.addControl(new BMap.disableDoubleClickZoom())
